...
<?php $nonce = create_csrf(); ?>
    ...
    <h1 class="text-capi lato-light text-center super3">Support Messages</h1>
...
<tr>
  ...
  <td><?php echo htmlentities($message->subject);?></td>
    <td><?php echo htmlentities(get_user_name_by_id($message->user_id));?></td>
  <td><?php echo htmlentities($message->message);?></td>
  <td><?php echo htmlentities(convert_time($message->created_at,2));?></td>
  <td><?php echo htmlentities($message->type);?></td>
  <td><a class="btn btn-sm" href="<?php echo htmlentities(BASE_URL ."messages/deleteSupport/" .$message->id)?>"><span class="icon icon-trash icon-2x"></span></a>
  <a class="btn btn-sm" href="<?php echo htmlentities(BASE_URL ."messages/newMessage/" .$message->user_id)?>"><span class="icon icon-reply icon-2x"></span></a></td>
